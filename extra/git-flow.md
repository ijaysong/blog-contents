...
해당 옵션을 지정하여 브랜치를 병합하면 해당 브랜치의 커밋 전체를 통합한 커밋이 추가된다.
토픽 브랜치 안의 커밋을 한꺼번에 모아서 통합 브랜치에 병합하고자 할 때 사용된다.

다음과 같은 흐름으로 커밋을 수정해본다고 하자.

~~~
master 브랜치 : A -> B -> C -> D -> X+Y
issue 브랜치  :      | -> X -> Y ----|
~~~

issue 브랜치의 모든 커밋을 하나의 커밋으로 병합하여 master 브랜치로 가져온다고 해보자.
...
#### 버전 관리 시스템
CLI에서 이루어지던 git의 영역을 확장해 UI까지 함께 제공하는 버전 관리 시스템이 있다.
이들을 사용함으로서 프로젝트의 이력과 흐름을 파악하는 것이 용이해진다.
* SourceTree
* TortoiseSVN

#### 프로젝트 관리 시스템
이외에도 프로젝트 단위로 수행하는 일감을 효율적으로 관리하기 위한 프로젝트 관리 시스템이 있다.
* Gitlab
* Redmine

프로젝트를 진행하는데 필요한 도구들을 제공하는 서비스로 주로 아래와 같은 서비스가 포함된다.

일감 - 업무의 내용을 기록, 담당자의 지정을 할 수 있게해서 업무를 효율적으로 관리할 수 있도록 하는 기능
위키 - 위키의 형식으로 지식을 저장
