...
ex 1)
맨 앞에 r이 위치해야 하고, r 뒤에 문자가 없어도 되고 아무리 많아도 상관 없다. 
...
ex 1)
맨 앞에 r이 위치해야 하고, r 뒤에 문자가 하나 있어야 하고 아무리 많아도 상관 없다. 
...
정규 표현식에서 대문자와 소문자는 전혀 다른 의미를 가진다.
\W는 \w와 정 반대의 의미를 가진다.
알파벳, 숫자, _ 가 아닌 문자열을 매칭한다.
[^A-z0-9_]와 의미가 같다.
...
워드 바운더리는 패턴 앞에 있느냐, 뒤에 있느냐 에 따라서 결과가 다르다. 

- \d\w : 워드 앞에 바운더리가 붙으면, 워드로 시작하는 단어 앞부분만 매칭된다.
- \w\d : 워드 위에 바운더리가 붙으면, 워드로 끝나는 단어 끝부분만 매칭된다.
