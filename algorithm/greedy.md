...
외판원 문제는 서로 다른 도시(5개)를 모두 지나는 최단 경로를 찾아내야 한다.
최단 경로를 찾기 위해서 가능한 모든 경로를 계산해야 한다.
...
=> 도시가 2개인 경우 = 가능한 경로는 2개

예를 들어 서울에 살며, 다른 도시를 돌아야 한다면 첫번째 도시는 서울이 된다.
하지만 때때로 첫 번째 도시(출발점)가 정해지지 않을 수도 있기 때문에 어떤 도시에서 출발하는지 알 필요가 있다.

#### 도시가 세 개인 경우
이제 도시가 하나 더 있다고 해보자. 가능한 경로는 몇개일까
...
=> 도시가 3개인 경우 = 가능한 경로는 6개

#### 도시가 네 개인 경우
도시가 하나 더 추가 되어 대구까지 고려한다.

<서울에서 출발 : 대구가 두번째 도시인 경우>
1. 서울 -> 대구 -> 전주 -> 순천
2. 서울 -> 대구 -> 순천 -> 전주

<서울에서 출발 : 전주가 두번째 도시인 경우>
3. 서울 -> 전주 -> 대구 -> 순천
4. 서울 -> 전주 -> 순천 -> 대구

<서울에서 출발 : 순천이 두번째 도시인 경우>
5. 서울 -> 순천 -> 대구 -> 전주
6. 서울 -> 순천 -> 전주 -> 대구

서울에서 출발하는 경로는 6개이다.
그런데 앞에서 도시가 3개인 경우에 계산한 것과 거의 같다. (도시가 하나 추가된 것만 빼면!)

만약 도시가 4개이면 먼저 하나를 고르면 3개가 남는다.
3개의 도시가 있으면 경로가 6개라는 것을 알고 있기 때문에 서울에서 출발하는 경로는 6개이다.

다른 도시에서 출발하는 경우도 마찬가지이다.
=> 대구에서 출발 = 가능한 경로는 6개
=> 전주에서 출발 = 가능한 경로는 6개
=> 순천에서 출발 = 가능한 경로는 6개

출발할 수 있는 도시가 4개 이고, 각 도시에서 출발하는 경로가 6개이므로 전체 4x6=24개의 경로가 있다.
