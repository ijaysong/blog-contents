...
도시 갯수에 따라 가능한 경로를 구하는 패턴이 있다.
|도시의 수 | 패턴                                                          |
|--------|--------------------------------------------------------------|
|1       |1개의 경로                                                       |
|2       |출발 가능한 도시는 2개 * 각 도시에서 출발하는 1개의 경로 (전체 2개의 경로)    |
|3       |출발 가능한 도시는 3개 * 각 도시에서 출발하는 2개의 경로 (전체 6개의 경로)    |
|4       |출발 가능한 도시는 4개 * 각 도시에서 출발하는 6개의 경로 (전체 24개의 경로)   |
|5       |출발 가능한 도시는 5개 * 각 도시에서 출발하는 24개의 경로 (전체 120개의 경로) |

만약 도시가 6개이면 가능한 경로는 720개.
도시가 7개이면 가능한 경로는 5,040개, 도시가 8개이면 경로는 40,320개이다.
이것을 `팩토리얼 함수`라고 한다.
도시의 수가 늘어나면 경로의 수가 엄청 빠르게 증가한다.
그래서 도시의 수가 많을수록 외판원 문제를 풀기 어려워진다.

외판원 문제와 집합 커버링 문제에는 공통점이 있다.
가능한 모든 경우를 다 따져서 최단/최소를 구해야 한다는 것이다.
이런 문제를 `NP-완전 문제`라고 한다.

## 어떤 문제가 NP-완전 문제인지 알 수 있는 방법은?
* 항목이 적을 때는 알고리즘이 빠르지만, 항목이 늘어나면서 갑자기 느려지는 것
* `X의 모든 조합`이라고 하면 보통 NP-완전 문제이다.
* 더 작은 하위 문제로 변환할 수 없어서 X의 가능한 모든 버전을 계산해야 한다면 아마도 NP-완전 문제 일것
* 문제가 수열(외판원 문제와 같은 도시의 순서 길이)를 포함하고, 풀기가 어려우면 NP-완전 문제일 수 있다.
